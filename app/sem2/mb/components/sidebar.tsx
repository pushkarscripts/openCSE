...
export default function Sidebar() {
  ...
  const chapters = [
    ...
    { id: "ch1", title: "Introduction to Biology" },
    { id: "ch2", title: "Genetics and Cytology" },
    { id: "ch3", title: "Biomolecules And Animal Physiology" },
    { id: "ch4", title: "Metabolism" },
    { id: "ch5", title: "Microbiology" },
  ];
      ...
      <div className="flex sticky top-14 z-40 h-[calc(100vh-3.5rem)] w-[50px] md:w-auto pointer-events-none md:pointer-events-auto">
        ...
        >
          ...
          <ul className="flex-1 overflow-y-auto space-y-0">
            {chapters.map((ch) => {
              const active = pathname === `/sem2/mb/${ch.id}`;
              return (
                <li key={ch.id}>
                  <Link
                    href={`/sem2/mb/${ch.id}`}
